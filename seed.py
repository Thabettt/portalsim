...
def seed_database():
    ...
    with Session(engine) as session:
        ...
        for i, course in enumerate(course_objs):
            ...
            # Create schedule (e.g. week 1..12)
            for week in range(1, 13):
                session.add(CourseSchedule(course_id=course.id, week_number=week, weekday=(i % 5)))
        session.commit()

        # 5. Create 100 Students (+ 1 test fixture student)
        logger.info("Creating 100 Students...")
        students = []
        
        # Primary fixed students for API backwards compatibility
        FIXED_STUDENT_DATA = [
        ...
        for fname, stu_id, email, is_for in FIXED_STUDENT_DATA:
            student = User(
                student_id=stu_id,
                email=email,
                full_name=f"{fname} Hassan",
        ...
        # Test user fixture for remark tests
        ...
        student_enrollments = {}  # student.id -> list of course objects

        for idx, student in enumerate(all_student_objs):
            # Enroll each student in 4 to 6 courses deterministically
            num_courses = 4 + (idx % 3)
        ...
        # 7. Generate Attendance Records & Warning Levels
        ...
        for student in all_student_objs:
            ...
            for c_idx, course in enumerate(enrolled):
                total_weeks = 12
                # Vary absent rates to generate a natural warning level distribution
                if (student.id + c_idx) % 15 == 0:
                    absent_count = 4  # Final warning range
                elif (student.id + c_idx) % 9 == 0:
                    absent_count = 3  # Second warning range
                elif (student.id + c_idx) % 5 == 0:
                    absent_count = 2  # First warning range
                else:
                    absent_count = random.choice([0, 1])  # None warning range

                absent_weeks = set(range(1, absent_count + 1))
                
                # Insert weekly sessions
                for week in range(1, total_weeks + 1):
                    ...
                    # Calculate cumulative warning level
        ...
        # 8. Generate Payments (Tuition, Lab Fee, Library Fee, Exam Fee, Exam Remark)
        ...
        for idx, student in enumerate(all_student_objs):
            ...
            # Exam Remark Charge for select students
            if idx % 7 == 0 or student.student_id == "STU-9999-0001":
                session.add(Payment(
                    student_id=student.id,
        ...
        # 9. Generate Assessments (Quizzes, Midterms, Finals, Projects)
        ...
        logger.info("Generating Internship records for all 100 students matching business rules...")
        
        # Exact Distribution Rules for 100 Students:
        # Index 0..19   (20 students) -> 20% Pending
        # Index 20..29  (10 students) -> 10% Rejected
        # Index 30..99  (70 students) -> 70% Accepted
        #   - Index 30..78 (49 students, ~70% of Accepted) -> < 8 accepted reports (Academic Fulfillment = Waiting)
        #   - Index 79..99 (21 students, ~30% of Accepted) -> >= 8 accepted reports (Academic Fulfillment = Fulfilled)
        #     - Index 79..90 (12 students) -> Eval form uploaded -> Career Center Final Status = Fulfilled
        #     - Index 91..99 (9 students)  -> Waiting CC final review -> Career Center Final Status = Waiting

        start_date = date(2024, 1, 15)
        end_date = date(2024, 6, 30)  # 5.5 months duration (>= 5 months rule!)
        ...
        for i, student in enumerate(students):
            ...
            # -------------------------------------------------------------
            # Category 1: PENDING INTERNSHIPS (Indices 0 to 19 - 20%)
            # -------------------------------------------------------------
            if i < 20:
                internship = Internship(
                    ...
                    description=f"5-month internship focusing on {f_major} projects and software development.",
                    ...
                    proof_of_acceptance_uploaded_at=None,
                ...
                # 0 progress reports for pending

            # -------------------------------------------------------------
            # Category 2: REJECTED INTERNSHIPS (Indices 20 to 29 - 10%)
            # -------------------------------------------------------------
            elif i < 30:
                reason = REJECTION_REASONS[(i - 20) % len(REJECTION_REASONS)]
                internship = Internship(
                    student_id=student.id,
                    company_name=org_name,
                    position=f"{f_major} Intern",
                ...
                session.add(internship)
                # 0 progress reports for rejected

            # -------------------------------------------------------------
            # Category 3: ACCEPTED INTERNSHIPS (Indices 30 to 99 - 70%)
            # -------------------------------------------------------------
            else:
                proof_date = datetime.utcnow() - timedelta(days=120)
                
                # Sub-category 3A: < 8 Accepted Progress Reports (Indices 30 to 78 - 49 students)
                if i < 79:
                    num_reports = 1 + ((i - 30) % 7)  # 1 to 7 reports
                ...
                # Sub-category 3B: >= 8 Accepted Progress Reports (Indices 79 to 99 - 21 students)
                else:
                    num_reports = 8 + ((i - 79) % 3)  # 8, 9, or 10 reports
                    academic_fulfillment = "Fulfilled"
                    
                    if i <= 90:  # 12 students -> Full Career Center Fulfillment!
                        eval_date = datetime.utcnow() - timedelta(days=10)
                        career_final = "Fulfilled"
                        int_status = InternshipStatus.COMPLETED
                    else:  # 9 students -> Academic Fulfilled, CC Waiting
                        eval_date = None
                        career_final = "Waiting"
                ...
                internship = Internship(
                    ...
                    proof_of_acceptance_uploaded_at=proof_date,
                ...
                # Add Progress Reports for Accepted Internship
                for r_num in range(1, num_reports + 1):
                    ...
                    session.add(InternshipProgressReport(
                        ...
                        report_number=r_num,
                        summary=f"Progress report #{r_num}: Completed bi-weekly milestones, submitted code reviews and unit tests.",
        ...
        # Create Internship for Test User (STU-9999-0001) as well
        test_internship = Internship(
            ...
            proof_of_acceptance_uploaded_at=datetime.utcnow() - timedelta(days=110),
            evaluation_form_uploaded_at=datetime.utcnow() - timedelta(days=10),
            career_center_review_status="Accepted",
            supervisor_review_status="Accepted",
        ...
        total_attendance = session.exec(select(Attendance)).all()
        ...
        logger.info(f"Total Attendance Records: {len(total_attendance)}")
        logger.info(f"Total Payment Records: {len(total_payments)}")
        logger.info(f"Total Assessment Records: {len(total_assessments)}")
        logger.info("==================================================")
